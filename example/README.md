...
断点修正：规律性的虚线短划间隔不再作为数据分段；Fig. 9 中颜色阈值造成的
短缺列仅在缺口内找到真实支持像素时补回。图例遮挡、同色曲线重合及没有像素
证据的区段仍保留为空白，不进行插值或隐藏数据推断。
...
- 当前候选：339 行，覆盖 6 个系列。µCT a22 使用论文 Table 3 只辅助在真实蓝色像素分支中选轨；表格值未被直接写入图像候选。
- 交叉核对：µCT a11、a22、a33 分别匹配 19、18、19 个 Table 3 位置，平均绝对误差分别约为 0.0108、0.0090、0.0011。
