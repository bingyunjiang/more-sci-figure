...
这些项目是并列能力目录：各自独立安装、独立运行、独立验收。`more-sci-figure` 不把其他 `more-*`
项目作为前置依赖，也不会自动把图表结果推入其他 skill。
...
当任务对象是论文图片、PDF 嵌入图表、已有 CSV/Excel 数据、曲线/散点/柱图/极坐标提取、异常复核或论文级重绘时，使用 `more-sci-figure`。如果任务目标是完整论文写作、新闻简报或漫画数字化，请使用对应项目；本项目只交付可审计的图表数据与图形产物。
...
- **v0.3.1 · 2026-07-26（当前版本）**
  ...
  - 补充独立使用边界，说明 More 系列是并列能力目录，不会自动跨 skill 推送图表结果。
- **v0.3.0 · 2026-07-26**
